  ...
  <v-form v-model="valid">
    <v-layout wrap justify-space-between>
      ...
      <v-flex xs12 md4>
        <upload-input
          title="Foto Garage"
          :file-changed-callback="fileGarage"
        ></upload-input>
      </v-flex>
      ...
      <v-flex xs12 md4>
        <upload-input
          ...
          :file-changed-callback="fileAddress"
        ></upload-input>
      </v-flex>
    </v-layout>
  </v-form>
...
export default {
  ...
  methods: {
    ...
    openVerify(val) {
      ...
      this.objSend = {
        ...this.objSend,
        driverId: this.$store.getters["lead/driverId"]
      };
      ...
      const success = dataDoc => {
        this.$store.commit("lead/setSteps", {
          SEND_DOCUMENTS: { complete: true, invalid: false }
        });
      ...
      axios
        .post(`${process.env.VUE_APP_URL_PREFIX}/documents`, form_data)
        .then(res => {
          sendGraph(infoDoc);
        })
        .catch(e => _error());
